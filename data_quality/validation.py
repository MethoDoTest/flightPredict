...
def validate_row(row):
    ...
    return validate_airline(row['Airline']) and \
        validate_source_destination(row['Source'], row['Destination']) and \
        validate_total_stops(row['Total_Stops']) and \
        validate_price(row['Price']) and \
        validate_date(row['Date'], row['Month'], row['Year']) and \
        validate_time(row['Dep_hours'], row['Dep_min']) and \
        validate_time(row['Arrival_hours'], row['Arrival_min']) and \
